...
public:
	...
	char buf[1500];		// actual packet with header 
};
...
private:
	...
	int send_data_pckt(Packet* p);// char* buf, unsigned int buf_size);
	int recv_data_pckt();
	...
	UINT worker_thread(LPVOID worker_params);
...
public:
	...
	unsigned int sender_base;
	unsigned int current_seq_no;
	unsigned int nextSeq;

	unsigned int no_packets_pending;
