// RDT3.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
UINT stat_thread(LPVOID stat_params) {
	...
	while (!p->stop) {
		...
		base_diff = ss->sender_base - last_base_no;
		//UINT64 diff = ss->bytes_acked - bytes_before;

		double speed = ((base_diff * 8 * (MAX_PKT_SIZE - sizeof(SenderDataHeader))) / 1e6)/2;
